...
const STARTER_ORANGE_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Orange Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".orange/settings.local.json", ".orange/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let orange_dir = cwd.join(".orange");
    artifacts.push(InitArtifact {
        name: ".orange/",
        status: ensure_dir(&orange_dir)?,
    });

    let orange_json = cwd.join(".orange.json");
    artifacts.push(InitArtifact {
        name: ".orange.json",
        status: write_file_if_missing(&orange_json, STARTER_ORANGE_JSON)?,
    });
    ...
    let orange_md = cwd.join("ORANGE.md");
    let content = render_init_orange_md(cwd);
    artifacts.push(InitArtifact {
        name: "ORANGE.md",
        status: write_file_if_missing(&orange_md, &content)?,
    });
...
pub(crate) fn render_init_orange_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# ORANGE.md".to_string(),
        String::new(),
        "This file provides guidance to Orange Code (orangecode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.orange.json`; reserve `.orange/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `ORANGE.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_orange_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("orange-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".orange/         created"), "Rendered: {}", rendered);
        assert!(rendered.contains(".orange.json     created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("ORANGE.md        created"));
        assert!(root.join(".orange").is_dir());
        assert!(root.join(".orange.json").is_file());
        assert!(root.join("ORANGE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".orange.json")).expect("read orange json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".orange/settings.local.json"));
        assert!(gitignore.contains(".orange/sessions/"));
        let orange_md = fs::read_to_string(root.join("ORANGE.md")).expect("read orange md");
        assert!(orange_md.contains("Languages: Rust."));
        assert!(orange_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("ORANGE.md"), "custom guidance\n").expect("write existing orange md");
        fs::write(root.join(".gitignore"), ".orange/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("ORANGE.md        skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".orange/         skipped (already exists)"));
        assert!(second_rendered.contains(".orange.json     skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("ORANGE.md        skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("ORANGE.md")).expect("read existing orange md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".orange/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".orange/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_orange_md(Path::new(&root));
