...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("ORANGE.md"),
            dir.join("ORANGE.local.md"),
            dir.join(".orange").join("ORANGE.md"),
            dir.join(".orange").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Orange instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Orange instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Orange Code settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".orange")).expect("nested orange dir");
        fs::write(root.join("ORANGE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("ORANGE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".orange")).expect("apps orange dir");
        fs::write(root.join("apps").join("ORANGE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".orange").join("instructions.md"),
            "apps dot orange instructions",
        )
        .expect("write apps dot orange instructions");
        fs::write(nested.join(".orange").join("ORANGE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".orange").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("ORANGE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("ORANGE.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.orange/ORANGE.md")),
            "ORANGE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("ORANGE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? ORANGE.md"));
    ...
    #[test]
    fn load_system_prompt_reads_orange_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".orange")).expect("orange dir");
        fs::write(root.join("ORANGE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".orange").join("settings.json"),
        ...
        let original_orange_home = std::env::var("ORANGE_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("ORANGE_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_orange_home {
            std::env::set_var("ORANGE_CONFIG_HOME", value);
        } else {
            std::env::remove_var("ORANGE_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_orange_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".orange")).expect("orange dir");
        fs::write(root.join("ORANGE.md"), "Project rules").expect("write ORANGE.md");
        fs::write(
            root.join(".orange").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Orange instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_orange_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".orange")).expect("nested orange dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".orange/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/ORANGE.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Orange instructions"));
