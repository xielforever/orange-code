...
Orange Code 是一个基于安全的 Rust 语言实现的本地编码代理（Agent）CLI 工具。本项目是开源项目 **[Orange Code](https://github.com/instructkr/orange-code)** 的二次开发版本。它继承了原项目优秀的架构设计（受 Claude Code 启发），现作为个人定制化工具进行独立维护与演进。

当前的主要产品是这个 Rust 工作区。它提供了一个名为 `orange`（或可通过配置更改）的二进制文件，支持从单一工作区运行交互式会话、感知工作区上下文的工具、本地代理工作流以及插件机制。
...
Orange Code is a local coding-agent CLI implemented in safe Rust. It is **Claude Code inspired** and developed as a **clean-room implementation**: it aims for a strong local agent experience, but it is **not** a direct port or copy of Claude Code.

The Rust workspace is the current main product surface. The `orange` binary provides interactive sessions, one-shot prompts, workspace-aware tools, local agent workflows, and plugin-capable operation from a single workspace.
...
```bash
cargo run --bin orange -- prompt "总结这个工作区"
cargo run --bin orange -- --model sonnet "审查最新代码更改"

# 或者运行已构建的二进制
./target/release/orange
```
...
本项目基于开源项目 [Orange Code](https://github.com/instructkr/orange-code) 源码进行二次开发，不隶属于原始开发团队。感谢原作者的开源贡献。
...
```bash
cargo run --bin orange -- --help
cargo run --bin orange --
cargo run --bin orange -- prompt "summarize this workspace"
cargo run --bin orange -- --model sonnet "review the latest changes"
```
...
```bash
./target/release/orange
./target/release/orange prompt "explain crates/runtime"
```
...
- Local agent and skill discovery with `orange agents` and `orange skills`
...
- Workspace-aware instruction/config loading (`ORANGE.md`, config files, permissions, plugin settings)
